defmodule CoinOrientation do
  ...
  def init do
    {:ok, %{"data" => %{
       ...
       message: %{
         description: [
           %{id: 0, text: "2人のプレイヤーがコインを同時に出し合うゲームを考える。"},
           %{id: 1, text: "2人の出した効果が「ともに表」か「ともに裏」である場合はプレイヤー1の勝ち、それ以外はプレイヤー2の勝ちとする。\nさらに、どちらのプレイヤーも表を出して勝つと、より高い利得が得られるものとする。"},
         ],
         experiment: "",
       },
       config: %{
         "max_round" => 1,
         "gain_table" => [[2, -2], [-1, 1], [-2, 2], [1, -1]],
         "askSnum" => false,
       },
       joined: 0,
     }}}
  ...
  def match(data) do
    ...
    reducer = fn {group, ids}, {participants, pairs} ->
      [id1, id2] = ids
      participants = participants 
                      |>Map.update!(id1, &updater.(&1, group, id2, "Player1"))
                      |>Map.update!(id2, &updater.(&1, group, id1, "Player2"))
  ...
  def handle_received(data, %{"action" => "submit answer", "params" => params}, id) do
    ...
    buddy_id = if participant.role == "Player1" do
      pair.user2
    else
      pair.user1
    end
    ...
    if participant.answer != nil and buddy.answer != nil do
      ...
      if participant.role == "Player1" do
