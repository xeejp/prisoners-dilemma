  ...
  render() {
    ...
    if(pairs && config && users){
      ...
      for(var key in pairs){
        ...
        datas[base + 0] = [key, (users[pairs[key].user1].snum != "")? users[pairs[key].user1].snum : pairs[key].user1, "Player1"].concat(pairs[key].logs.map(obj => (obj.answer1 == "yes")? "表" : "裏").reverse())
        datas[base + 1] = [  "", (users[pairs[key].user2].snum != "")? users[pairs[key].user2].snum : pairs[key].user2, "Player2"].concat(pairs[key].logs.map(obj => (obj.answer2 == "yes")? "表" : "裏").reverse())
      }
    }
    console.log(datas)
    return (
