  ...
  render() {
    ...
    if(pairs && config && users){
      ...
      for(var key in pairs){
        ...
        datas[base + 0] = [key, (users[pairs[key].user1].snum != "")? users[pairs[key].user1].snum : pairs[key].user1, "彼"].concat(pairs[key].logs.map(obj => (obj.answer1 == "yes")? "待つ" : "出掛ける").reverse())
        datas[base + 1] = [  "", (users[pairs[key].user2].snum != "")? users[pairs[key].user2].snum : pairs[key].user2, "彼女"].concat(pairs[key].logs.map(obj => (obj.answer2 == "yes")? "待つ" : "出掛ける").reverse())
      }
    }
    console.log(datas)
    return (
